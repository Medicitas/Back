...
const createUser = async (req, res) => {
  const {name, last_name, phone, password, email} = req.body;
  ...
  const createUserQuery = `INSERT INTO
      "public"."User"(name, last_name, phone, password, email,rol_id,user_id)
      VALUES($1, $2, $3, $4, $5, $6)
      returning *`;
  const values = [
    name,
  ...
  try {
    ...
    if (error.routine === '_bt_check_unique') {  // ver si ya existe
...
const signinUser = async (req, res) => {
  ...
  const signinUserQuery = 'SELECT * FROM "public"."User" WHERE email = $1';
